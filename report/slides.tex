...
\begin{center}
\includegraphics[width=0.98\textwidth]{../plots/benchmark_dvn_vs_greedy_vs_random_20260313_230003.png}
\end{center}
\footnotesize Same benchmark figure used in the report for 1P performance context.
\end{column}
...
\vspace{0.3em}
\begin{block}{Richer Conditioning}
PPO value $V_\phi(s_{t+3})$ sees full context at terminal state, not isolated afterstate.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
%-----------------------------------------------------------------------------
\begin{frame}{PPO Training Figure: Value Weight Sweep}
\begin{center}
\includegraphics[width=0.90\textwidth]{../plots/value_weight_sweep.png}
\end{center}
...
Sweep over \(\alpha\) in
\[
\text{score} = \text{symlog}(R_{t+1:t+3}) + \alpha\,\text{symlog}(V_\phi(s_{t+3}))
\]
Best region in the report: \(\alpha \in [0.1, 0.5]\).
\end{frame}
