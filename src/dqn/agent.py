...
class RainbowAgent1P(BaseAgent):
    def __init__(self, action_size=64, lr=1e-4, gamma=0.99, buffer_size=10000, 
        ...
        from .models import RainbowCNNNet1P
    ...
    def select_action(self, state, epsilon=0.0):
        # noisy nets handle exploration, epsilon is ignored
        valid_mask = state['valid_placements'].flatten()
        valid_actions = np.flatnonzero(valid_mask)
        ...
        with torch.no_grad():
            ...
            probs = self.policy_net(board, piece, valid)
    ...
    def update_model(self):
        ...
        if len(batch[0]) == 1:  # somehow sampled a 0-init from numpy
            return None
